...
{
    public class OrderConverter : IConverter
    {
        public DocumentViewModel Fix(string name, byte[] bytes, List<DocumentValidationError> errors, out byte[] stream)
        {
            ...
            using (var _ms = new MemoryStream())
            {
                ...
                {
                    ///последней сторокой должна быть таблица с подписью и фамилией заверяющего
                    ///
                    if (doc.MainDocumentPart.Document.Body.ChildElements.Where(x => x is Paragraph || x is DocumentFormat.OpenXml.Wordprocessing.Table).Last().GetType() != typeof(DocumentFormat.OpenXml.Wordprocessing.Table))
                    {
                        doc.MainDocumentPart.Document.Body.RemoveChild<Paragraph>(doc.MainDocumentPart.Document.Body.ChildElements.OfType<Paragraph>().Where(x => x is Paragraph).Last());
                    }
                    var paragraphs = doc.MainDocumentPart.Document.Body.OfType<Paragraph>().Where(x => x.InnerText.Trim() != "").ToList();
                    ///Распоряжение - слева шрифт 16 ариал жирный
                    ///
                    bytes = corrector.CorrectProperties(bytes, paragraphs[0].ParagraphId, new Rules() { Font = "Arial", FontSize = 32.0f, LineSpacing = 1.5f, FontStyle = new Bold(), ParagraphSpacing = 1.0f });
                    ///подразделение - слева шрифт 10.5 ариал жирный
                    ///
                    bytes = corrector.CorrectProperties(bytes, paragraphs[1].ParagraphId, new Rules() { Font = "Arial", FontSize = 21.0f, LineSpacing = 1.5f, FontStyle = new Bold(), ParagraphSpacing = 1.0f });
                    ///название распоряжения по центру жирный
                    ///
                    bytes = corrector.CorrectProperties(bytes, paragraphs[3].ParagraphId, new Rules() { Font = "Times New Roman", FontSize = 24f, LineSpacing = 1.5f, FontStyle = new Bold(), Justification = JustificationValues.Center, ParagraphSpacing = 1.0f });
                }
            }
        ...
        public List<DocumentValidationError> Validate(WordprocessingDocument doc)
        {
            ...
            ///
            if (doc.MainDocumentPart.Document.Body.ChildElements.OfType<Paragraph>().Where(x => x.ParagraphProperties != null && x.ParagraphProperties?.NumberingProperties != null).ToList().Count() == 0)
            {
                result.Add(new DocumentValidationError() { ParagraphId = new HexBinaryValue("-1"), ErrorType = ErrorType.GridError, Position = -1, Description = "В документе должен быть нумерованный список" });
            }

            var paragraphs = doc.MainDocumentPart.Document.Body.OfType<Paragraph>().Where(x => !Regex.IsMatch(x.InnerText.Trim(), @"^\s{0,}$") ).ToList();

            
            ///если в документе одно приложение - оно не нумеруется
            ///
            ///ищем параграфы с текстом "Приложение 1-9"
            ///если их количество 1 - проверяем нумеровано или нет
            ///
            if (paragraphs.Where(x => Regex.IsMatch(x.InnerText, @"Приложение(\s[0-9]+)?")).Count() == 1)
            {
                foreach (var paragraph in paragraphs.Where(x => x.InnerText.Contains("Приложение")).ToList())
                {
                    if (Regex.IsMatch(paragraph.InnerText, @"Приложение\s[0-9]+к распоряжению", RegexOptions.IgnoreCase))
                    {
                        result.Add(new DocumentValidationError() { ParagraphId = new HexBinaryValue(paragraph.ParagraphId.ToString()), ErrorType = ErrorType.GridError, Position = -1, Description = "Единственное приложение не нумеруется" });
                    }
                }
            }

            ///Первый параграф должен содержать только слово Распоряжение
            ///
            if (!Regex.IsMatch(paragraphs[0].InnerText, @"\s{0,}РАСПОРЯЖЕНИЕ\s{0,}"))
            {
                result.Add(new DocumentValidationError() { ParagraphId = paragraphs[0].ParagraphId, ErrorType = ErrorType.GridError, Position = 0, Description = $"Некорректный формат документа в позиции 0 - Необходимо \"РАСПОРЯЖЕНИЕ\"" });
            }
            ///второй параграф - наименование подразделения
            ///
            if (!Regex.IsMatch(paragraphs[1].InnerText, @"Управление внутреннего аудита", RegexOptions.IgnoreCase))
                result.Add(new DocumentValidationError() { ParagraphId = paragraphs[1].ParagraphId, ErrorType = ErrorType.GridError, Position = 1, Description = $"Некорректный формат документа в позиции 1 - Необходимо наименование подразделения" });

            
            ///3 параграф - дата и номер распоряжения
            ///
            if (!Regex.IsMatch(paragraphs[2].InnerText, @"[0-9.\-/]+", RegexOptions.IgnoreCase))
                result.Add(new DocumentValidationError() { ParagraphId = paragraphs[3].ParagraphId, ErrorType = ErrorType.GridError, Position = 3, Description = $"Некорректный формат документа в позиции 3 - Необходимо дата и номер распоряжения" });
            if (!Regex.IsMatch(paragraphs[2].InnerText, @"^№\s?(.)*", RegexOptions.IgnoreCase))
                result.Add(new DocumentValidationError() { ParagraphId = paragraphs[3].ParagraphId, ErrorType = ErrorType.GridError, Position = 3, Description = $"Некорректный формат документа в позиции 3 - Необходимо дата и номер распоряжения" });

            ///4 строка - название распоряжения по центру
            ///
            if (paragraphs[3].ParagraphProperties == null)
            {
                result.Add(new DocumentValidationError() { ParagraphId = paragraphs[3].ParagraphId, ErrorType = ErrorType.GridError, Position = 4, Description = $"Некорректный формат документа в позиции 4 - Необходимо название распоряжения форматированное по центру" });
            }
            else if (paragraphs[3].ParagraphProperties.Justification == null)
            {
                result.Add(new DocumentValidationError() { ParagraphId = paragraphs[3].ParagraphId, ErrorType = ErrorType.GridError, Position = 4, Description = $"Некорректный формат документа в позиции 4 - Необходимо название распоряжения форматированное по центру" });
            }
            else if (paragraphs[3].ParagraphProperties.Justification.Val != JustificationValues.Center)
                result.Add(new DocumentValidationError() { ParagraphId = paragraphs[3].ParagraphId, ErrorType = ErrorType.GridError, Position = 4, Description = $"Некорректный формат документа в позиции 4 - Необходимо название распоряжения форматированное по центру" });
            ...
            foreach (var paragraph in paragraphs)
            {
                    ...
                    result.Add(new DocumentValidationError() { ParagraphId = paragraph.ParagraphId, ErrorType = ErrorType.StyleError, Position = -1, Description = $"Отсутствует форматирование текста параграфа {paragraph.ParagraphId}" });
                {
                    ...
                    ///
                    if (paragraph.ParagraphProperties == null || paragraph.ParagraphProperties.SpacingBetweenLines == null) result.Add(new DocumentValidationError() { ParagraphId = paragraph.ParagraphId, ErrorType = ErrorType.StyleError, Position = -1, Description = $"Отсутствует форматирование текста параграфа {paragraph.ParagraphId}, межстрочный интервал" });
                    else
                    {
                        ...
                        if (paragraph.ParagraphProperties.SpacingBetweenLines.Before != new StringValue((rule.ParagraphSpacing * 240).ToString()))
                            result.Add(new DocumentValidationError() { ParagraphId = paragraph.ParagraphId, ErrorType = ErrorType.StyleError, Position = -1, Description = $"Отсутствует форматирование текста параграфа {paragraph.ParagraphId}, абзацный отступ перед, ожидаемое значение {rule.ParagraphSpacing * 240}" });
                        ...
                        if (paragraph.ParagraphProperties.SpacingBetweenLines.After != new StringValue((rule.ParagraphSpacing * 240).ToString()))
                            result.Add(new DocumentValidationError() { ParagraphId = paragraph.ParagraphId, ErrorType = ErrorType.StyleError, Position = -1, Description = $"Отсутствует форматирование текста параграфа {paragraph.ParagraphId}, абзацный отступ после, ожидаемое значение {rule.ParagraphSpacing * 240}" });
                    }
                        ...
                        else
                        {
                            ...
                            else
                            {
                                if (run.RunProperties.FontSize?.Val != new StringValue(rule.FontSize.ToString()))
                                    result.Add(new DocumentValidationError() { ParagraphId = paragraph.ParagraphId, ErrorType = ErrorType.StyleError, Position = -1, Description = $"Отсутствует форматирование текста параграфа {paragraph.ParagraphId}, размер шрифта, ожидаемое значение {rule.FontSize}" });
                            }
                            ...
                            else
                            {
                                if (run.RunProperties.RunFonts?.Ascii != new StringValue(rule.Font))
                                    result.Add(new DocumentValidationError() { ParagraphId = paragraph.ParagraphId, ErrorType = ErrorType.StyleError, Position = -1, Description = $"Отсутствует форматирование текста параграфа {paragraph.ParagraphId}, размер шрифта, ожидаемое значение {rule.Font}" });
                            }
